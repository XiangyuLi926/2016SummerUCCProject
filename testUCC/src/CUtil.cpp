...
#include <algorithm>
#include <cstring>
#include <ctime>     // Modification: 2015.12
...
#if defined(UNIX) && !defined(__MINGW32__)    // Modification: 2009.01 2015.12
	#include <dirent.h>
	#include <sys/stat.h>
	#define	UNIX_FILESYS_AVAILABLE
#else
	// Windows OS or
	// MinGW is (mostly) UNIX style as Win32 binary.  No Links for Windows file system.
	#include <direct.h>
	#include <io.h>
#endif
...
void CUtil::InitToLower()
{
	...
	for ( int j = 0; j < (int)( ( sizeof( lowerChars ) / sizeof( lowerChars[0] ) ) ); j++ )
	{
...
#ifdef UNIX_FILESYS_AVAILABLE           // Modification: 2011.10  2015.12
...
{
    string fullPath;
#ifdef UNIX_FILESYS_AVAILABLE                                                   // Modification: 2011.05
    DIR *dir;
...
void CUtil::SemanticFormat(string &statement)
{
    ...
    if(statement.find(concat_op_and) == string::npos && statement.find(concat_op_or) == string::npos){
        ...
        if(statement[0]=='!'){
            ...
            if( eq_pos != (unsigned int)(string::npos) )
			{
			...
			else if( ne_pos != (unsigned int)(string::npos) )
			{
