...
// Utilities to support checking the Heap.  Implemented for Windows
	...
	#define		SIMPLIFIED_SIGNAL_HANDLER
#endif
...
#define		MAX_CALL_FRAMES		128
	...
	#if !defined(__CYGWIN__)	// Cygwin does NOT have execinfo.h and so no UNIX style backtrace
		#define	UNIX_BACKTRACE_OK	1
		#include	<execinfo.h>

		void printStackTrace( FILE *out = stderr, unsigned int max_frames = MAX_CALL_FRAMES - 1);
	#else
		// Interface for Cygwin to use
		#define		SIMPLIFIED_SIGNAL_HANDLER
	#endif
#endif
...
#ifdef	SIMPLIFIED_SIGNAL_HANDLER
...
void ShowFileSetStats( const bool useListA, string * pString )
{
	...
	#if defined(__MINGW32__)
		printf( "Largest file is  %ld  Bytes.\n", largestFile );
	#else
